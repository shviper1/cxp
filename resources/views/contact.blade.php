            ...
            <div class="grid gap-8 lg:grid-cols-5">
                                ...
                                @if (!empty($detail['value']))
                                    <div>
                                        ...
                                        @if (!empty($detail['href']))
                                            <a href="{{ $detail['href'] }}"
                                                class="mt-1 inline-flex items-center text-base font-medium text-indigo-600 hover:text-indigo-700">
                                                {{ $detail['value'] }}
                                            </a>
                                        @else
                            ...
                            <div class="mt-4 flex flex-wrap gap-3">
                                @foreach ($socialLinks as $label => $url)
                                    <a href="{{ $url }}" target="_blank" rel="noopener"
                                        class="inline-flex items-center rounded-full border border-slate-200 px-4 py-2 text-sm font-medium text-slate-600 transition hover:border-indigo-500 hover:text-indigo-600">
                                        {{ $label }}
                                    </a>
                                @endforeach
                            </div>
                ...
                <div class="lg:col-span-3">
                    <div class="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
                        ...
                        <p class="mt-2 text-sm text-slate-600">Fill out the form below and we&rsquo;ll get back to you
                        ...
                        <form action="{{ route('contact.submit') }}" method="POST" class="mt-6 space-y-6">
                            ...
                            <div class="grid gap-6 sm:grid-cols-2">
                                <div>
                                    <label for="phone" class="block text-sm font-medium text-slate-700">Phone
                                        (optional)</label>
                            ...
                            <div>
                                ...
                                <textarea id="message" name="message" rows="5" required
                                    class="mt-2 w-full rounded-lg border border-slate-200 px-4 py-3 text-slate-900 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('message') }}</textarea>
                                @error('message')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <button type="submit"
                                class="inline-flex w-full items-center justify-center rounded-lg bg-indigo-600 px-6 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-600 focus:ring-offset-2 sm:w-auto">
                                Send message
                            </button>
                        </form>
                    </div>
