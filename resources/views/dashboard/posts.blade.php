...
<div class="max-w-6xl mx-auto p-6">
                ...
                <div class="p-6 hover:bg-gray-50 transition duration-200">
                    <div class="flex justify-between items-start">
                            ...
                            @if($post->media->count() > 0)
                                <div class="flex space-x-2 mt-3">
                                    @foreach($post->media->take(3) as $media)
                                        @if($media->type === 'image')
                                            <img src="{{ asset('storage/' . $media->file_path) }}" alt="Post media" class="w-16 h-16 object-cover rounded-lg border">
                                        @endif
                                    @endforeach
                                    @if($post->media->count() > 3)
                                        <div class="w-16 h-16 bg-gray-200 rounded-lg border flex items-center justify-center text-xs text-gray-600">
                                            +{{ $post->media->count() - 3 }}
                                        </div>
                                    @endif
                                </div>
                        ...
                        <div class="ml-6 flex flex-col space-y-2">
                            <button class="text-indigo-600 hover:text-indigo-800 text-sm font-medium" onclick="showPostDetails({{ $post->id }})">
                                View Details
                            </button>
    ...
    @else
        <div class="bg-white rounded-lg shadow-md p-12 text-center">
            ...
            <p class="text-gray-500 mb-6">You haven't submitted any posts yet. Create your first post to get started!</p>
            <a href="{{ route('posts.create') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-4 rounded-lg font-medium transition duration-200 inline-flex items-center">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                </svg>
