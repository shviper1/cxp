                ...
                @forelse($sections as $section)
                    <div class="transition-shadow bg-white border shadow-sm rounded-xl border-slate-200 hover:shadow-md">
                        <div class="p-6">
                                ...
                                <div class="mb-4 space-y-2">
                                    @foreach ($section->categories->take(3) as $category)
                                        <a href="{{ route('locations.section', [$country, $section]) }}"
                                            class="block px-3 py-2 text-sm transition-colors rounded-md text-slate-700 hover:bg-slate-50">
                                            {{ $category->name }}
                                        </a>
                                    @endforeach
                                    @if ($section->categories->count() > 3)
                                        <p class="px-3 text-xs text-slate-500">
                            ...
                            @else
                                <p class="mb-4 text-sm text-slate-500">No categories available in this section yet.</p>
                            @endif
                        </div>
                    </div>
                @empty
                    <div class="py-12 text-center col-span-full">
                        ...
                        <h3 class="mb-2 text-lg font-medium text-slate-900">No sections available</h3>
